...
testthat::test_that("create archive structure creates the appropriate folders", {
  ...
  archiveR::create_archive_str(output_dir)
...
testthat::test_that("move to current moves the files to current", {
  ...
  cd <- archiveR::create_current_dir(output_dir)
  archiveR::move_to_current(fls, cd)
...
testthat::test_that("archive zip files creates a zipped directory", {
  ...
  archive_dir <- archiveR::create_archive_dir(output_dir)
  current_dir <- archiveR::create_current_dir(output_dir)
  archiveR::create_archive(commit, current_dir, archive_dir)
